...
ls -la
./kubectl --kubeconfig=/dev/null --server=$K8S_EUROPE_SERVER --certificate-authority=cert-eur.crt --token=$K8S_EUROPE_TOKEN apply -f ./.k8s/manifest-europe.yml
...
./kubectl --kubeconfig=/dev/null --server=$K8S_ASIA_SERVER --certificate-authority=cert-asia.crt --token=$K8S_ASIA_TOKEN apply -f ./.k8s/manifest-asia.yml
